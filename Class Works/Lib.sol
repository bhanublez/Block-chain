...
//This code is written by one of my classmate this is not written by me
...
contract Library {
    ...
    modifier only_Librarian() {
        require(Librarian == msg.sender, "You are not permitted!!");
        _;
    }
    modifier not_librarian() {
        require(msg.sender != Librarian, "You are registered as librarian");
        _;
    }
    struct userRegisteration {
        string Uname;
        string DOJ;
        address Uaddress;
        uint256 No_of_Books;
    ...
    function Register(
        ...
        string memory _DOJ,
        address _Uaddress
    ) public not_librarian {
        userRegisteration memory newUser = userRegisteration(
            _Uname,
            _DOJ,
            _Uaddress,
            0,
    ...
    function Borrow(
        uint256 _Bid,
        uint256 _Uid
    ) public not_librarian returns (string memory) {
        require(Book_details[_Bid].isAvailable == true, "Book is Not Availble");
        require(
    ...
    function ReturnBook(uint256 _Bid, uint256 _Uid) public not_librarian {
        require(
            User_details[_Uid].isRegistered == true,
            "You're not registered"
        );
